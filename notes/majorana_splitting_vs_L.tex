...
wins over the linear growth.  The exact transfer-matrix calculation at
$t=\Delta$ keeps the $(1-\lambda^2)$ prefactor (it is \emph{not} subleading in
$L$; it only tends to $1$ as $\mu\to 0$), giving
\begin{equation}
  \boxed{E_0(L) \approx 2t\,(1-\lambda^2)\left(\frac{|\mu|}{2t}\right)^L,
  \qquad \lambda=\frac{|\mu|}{2t}.}
\end{equation}
Numerically (BdG, \texttt{src/block1.py}) the ratio $E_0/\lambda^L$ tends to
$2t(1-\lambda^2)$ exactly: $1.875,\,1.5,\,0.875$ at $\mu=0.5,\,1,\,1.5$
($t=1$), matching this form and \emph{not} the bare $2t\,\lambda^L$.
