...
\begin{equation}
  ...
  \;\approx\; 2t(1-\lambda^2)\lambda^{L},
  \qquad
  \lambda=\frac{|\mu|}{2t}=e^{-1/\xi}.
\end{equation}
...
\emph{biased}: it preferentially sends $\ket1\to\ket0$. In the repository
Jordan--Wigner convention $c_j^\dagger c_j=(I+Z_j)/2$, so $\ket0$ is the
occupied local state; the essential point is that the channel changes the local
$Z_j$ eigenvalue and can therefore leak the protected even state into the odd
sector rather than scrambling it symmetrically.
