...
\begin{itemize}
  \item \textbf{$T_1$ is parity poisoning.} In the repository Jordan--Wigner convention $c_j^\dagger c_j=(I+Z_j)/2$, so $\ket0$ is the occupied local state. Qiskit amplitude damping maps $\ket1\to\ket0$; the important point is that this changes the local $Z_j$ eigenvalue and can flip the global fermion parity, pushing the state out of the even sector the diagnostic lives in.
