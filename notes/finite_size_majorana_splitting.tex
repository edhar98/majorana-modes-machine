...
\begin{equation}
  \text{correct}: \quad
  \{E_n\}_{n=1}^{L}
  = \text{one copy from each adjacent pair in } \text{sort}\bigl(|e_i|\bigr).
\end{equation}
In code, replace
\begin{verbatim}
...
with
\begin{verbatim}
  return np.sort(np.abs(evals))[::2] # correct: |E| plus pair deduplication
\end{verbatim}
The absolute value is what protects the near-zero Majorana mode from a
floating-point sign flip. The stride is what removes the particle-hole double
counting: each physical quasiparticle energy appears twice as $\pm E$ before
