...
Current local-only/untracked worktree items observed on 2026-06-09 include `.cursorindexingignore`, `.notes.txt.swp`, `.specstory/`, `diff.tex`, `diff.txt`, `notes.txt`, `src/.block3.py.swp`, `tools/`, a Qiskit crash-course notebook checkpoint, generated presentation PDFs/VRB files for weeks 1-7, and `presentation/week5/slides_my.tex` / `presentation/week5/slides_my.pdf`. Do not delete or revert these unless explicitly asked.
