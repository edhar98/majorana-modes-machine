...
|---|---|---|---|---|
| 1 | **Kitaev Chain & Bulk Topology** | The infinite/bulk story: lattice Hamiltonian, Majorana representation, real-space→momentum BdG, dispersion $E(k)$, gap closing $\lvert\mu\rvert=2t$, winding number $\nu$, phase diagram. | 1–2 | _(unclaimed)_ |
| 2 | **Finite-Size Physics & Majorana Edge Modes** | The finite open chain / edge story: OBC spectrum, near-zero edge modes, exponential edge localization, hybridization splitting $E_0(L)\approx 2t(\lvert\mu\rvert/2t)^L$, the sign-filter artefact. | 2–3 | _(unclaimed)_ |
| 3 | **Qubit Encoding (Jordan–Wigner)** | The fermion→qubit mapping: JW term-by-term, qubit $H$ (Z/XX/YY), sweet spot $t=\Delta$, fermion-parity operator & sectors, parity-gap vs BdG cross-check, AFM-Ising comparison. | 4 | _(unclaimed)_ |
...
**finalize this seam between themselves once both rows are claimed**, and update
