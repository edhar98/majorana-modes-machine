...
\ai{This report covers Part 5, \emph{NISQ Noise on the Diagnostic}.  Week 8 first
kept the validated state frozen and tested only readout error, local
depolarization, and finite measurement counts.  Week 9 then applied noise during
the preparation circuit, where it accumulates after individual gates, and later
allowed the VQE optimizer itself to receive noisy cost values.  A recorded
backend calibration was also compared with the uniform test model.  Throughout,
the same $L=4$ circuit and edge-string observable are used so that the source of
each change remains clear.}
