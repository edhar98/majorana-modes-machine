...
\usepackage{graphicx}
\usepackage{subcaption} % FIX 2: Added subcaption package
\usepackage{xcolor}
\usepackage{tikz}
...
\begin{frame}{What Week 7 Gives Us}
  \begin{columns}[T]
    \begin{column}{0.62\textwidth}
      \centering
      % Assuming block3_week7_vqe_sweep.pdf is also in the plots folder
...
\begin{frame}{Circuit-Level Noise Protocol: Formalism}
    ...
    \begin{itemize}
        \item \textbf{Gate Noise ($\mathcal{E}_{\text{gate}}$):} Local two-qubit depolarizing channels after entangling gates, $\mathcal{D}_p(\rho) = (1-p)\rho + \frac{p}{15}\sum_{i=1}^{15} P_i \rho P_i$, over the non-identity two-qubit Paulis.
    ...
    \begin{columns}
        \begin{column}{0.45\textwidth}
            ...
            \begin{itemize}
                ...
                \item \textit{Readout error alone} scales the curve by $(1-2\epsilon)^4$ for independent symmetric readout flips on the four measured bits.
                \item \textit{Gate noise} lowers purity, $\text{Tr}(\rho^2)<1$, and suppresses coherent edge correlations, especially near $\mu_c=\pm2t$.
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \begin{center}
            \scriptsize Schematic expectation before simulator data
            \vspace{0.1cm}
...
\begin{frame}{Visualising the Noise}
\begin{figure}[htbp]
    \centering
    % First subfigure
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        % FIX 3: Removed redundant pathing
        \includegraphics[width=\textwidth]{Noise-epsilon0gate0.png}
        \caption{ReadoutError:0.0 ,  GateError:0.0}
        \label{fig:graph1}
    \end{subfigure}
    \hfill
    % Second subfigure
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        % FIX 3: Removed redundant pathing
        \includegraphics[width=\textwidth]{Noise-epsilon0.11gate0.07.png}
        \caption{ReadoutError:0.11,   GateError:0.7}
        \label{fig:graph2}
    \end{subfigure}
    \hfill
    % Third subfigure
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        % FIX 3: Removed redundant pathing
        \includegraphics[width=\textwidth]{Noise-epsilon0.25gate0.15.png}
        \caption{ReadoutError:0.25,   GateError:0.15}
        \label{fig:graph3}
    \end{subfigure}
\end{figure} % FIX 1: Closed the figure environment
\end{frame}
...
\begin{frame}{Phase-Boundary Failure Criterion}
    ...
    \vspace{0.4cm}
    \textbf{Failure Criterion:} The diagnostic fails when the measured contrast between the topological window and the trivial region cannot be resolved from the statistical and hardware noise floor with a $3\sigma$ confidence interval.
    
    \vspace{0.2cm}
    Specifically, failure occurs if the measurable contrast satisfies:
    \begin{equation}
        \max_{|\mu|<2t} |\langle \hat{O}_{\text{edge}}(\mu) \rangle_{\text{meas}}|
        -
        \max_{|\mu|>2t} |\langle \hat{O}_{\text{edge}}(\mu) \rangle_{\text{meas}}|
        \leq
        \frac{3}{\sqrt{N_{\text{shots}}}} + \delta_{\text{SPAM}}
    \end{equation}
