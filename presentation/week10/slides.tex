...
\begin{frame}{Block 4 --- Which Physical Errors Kill the Topology?}
  ...
  \vspace{0.1cm}
  \footnotesize
  \textbf{What to measure:} the string order $O_{\mathrm{edge}}$, the fermion parity $P=\prod_j Z_j$, and the splitting $\delta E=|E_{\mathrm{even}}-E_{\mathrm{odd}}|\sim e^{-L/\xi}$ --- exactly the three objects Blocks 2--4 already compute.
\end{frame}
...
\begin{frame}{Block 4 --- Robust Error: Local Disorder (Exact, to $L=32$)}
  \begin{columns}[T]
    \begin{column}{0.66\textwidth}
      \centering
      \includegraphics[width=\textwidth,height=0.66\textheight,keepaspectratio]{block4_disorder_robustness.pdf}
    \end{column}
    \begin{column}{0.34\textwidth}
      \small
      Charge noise $=$ a random field $\sum_j\delta h_j Z_j$ (parity-even). Free-fermion BdG with per-site $\mu_j$, exact.
      \begin{itemize}
      ...
      \end{itemize}
      \textbf{Local, parity-even, sub-gap $\Rightarrow$ topologically harmless.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Block 4 --- Fatal Error: Quasiparticle Poisoning}
  \begin{columns}[T]
    \begin{column}{0.66\textwidth}
      \centering
      \includegraphics[width=\textwidth,height=0.66\textheight,keepaspectratio]{block4_robust_vs_fatal.pdf}
    \end{column}
    \begin{column}{0.34\textwidth}
      \small
      ...
      \end{itemize}
      \textbf{QP poisoning is the Achilles heel}: the poisoning time must beat the gate time.
    \end{column}
  \end{columns}
\end{frame}
