...
def positive_spectrum(self):
    ...
    return np.sort(np.abs(evals))[::2]  # |E| plus one copy per +/-E pair
  \end{lstlisting}
...
\begin{frame}{Derivation: finite-size splitting $E_0(L)$}
  ...
  The exact transfer-matrix calculation ($t=\Delta$) keeps the $(1-\lambda^2)$ prefactor (with $\lambda=|\mu|/2t$):
  \[
    \boxed{E_0(L) \approx 2t\left(1-\lambda^2\right)\left(\frac{|\mu|}{2t}\right)^L}
  \]
\end{frame}
...
\begin{frame}{Block 2 — key takeaways}
  ...
  \end{enumerate}
  \vfill
  \textbf{Next session (Block 2 cont.):} Jordan–Wigner transform → qubit encoding → quantum circuit for ground-state preparation.
\end{frame}
...
\begin{frame}{Appendix D: BdG extraction — sign filter and double counting}
  ...
  \begin{center}
    \includegraphics[width=0.9\linewidth]{block1_09_npabs_comparison.pdf}
  \end{center}

  Taking $|E|$ fixes the near-zero sign ambiguity. For the \emph{full} positive
  spectrum, the sorted $|E|$ list still contains each physical quasiparticle
  energy twice, so the code keeps one from each adjacent pair:
  \[
    E_n=\texttt{np.sort(np.abs(evals))[::2]} .
  \]
\end{frame}
