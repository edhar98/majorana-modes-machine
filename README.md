...
Notes are kept as LaTeX sources in `notes/`. PDFs are generated by CI and linked from the main site index. The Block 4 notes are `notes/block4_noise_diagnostic.tex` (Week 8 frozen-state diagnostic) and `notes/week9_note.tex` (integrating gate errors via Qiskit: theory, the channel-equivalence verification, why the frozen-state model is not the real one, and topological protection).
