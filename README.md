...
## Block 4 — NISQ Reality Check

Block 4 continues from the validated Block 3 Hamiltonian and edge-string observable. The Week 8 noise plots are generated by `src/block4.py`; the exploratory notebook version is not the source of truth for slide figures. The corresponding merged notes are `notes/block4_noise_diagnostic.tex`, which separate the implemented frozen-state noise diagnostic from broader noise theory not implemented yet.
...
python block4.py --plots 2          # Week 9 circuit-level gate noise vs frozen-state model
python block4.py --plots 3          # Week 9 gate-error check + topological protection sweep
```
...
| 10 | Block 4 — NISQ reality check, physical-error taxonomy, project summary | [week10.pdf](https://edhar98.github.io/majorana-modes-machine/week10.pdf) |

## Notes

Notes are kept as LaTeX sources in `notes/`. PDFs are generated by CI and linked from the main site index. The Block 4 notes are `notes/block4_noise_diagnostic.tex` (frozen-state diagnostic) and `notes/circuit_level_gate_noise.tex` (integrating gate errors via Qiskit: theory, the channel-equivalence verification, why the frozen-state model is not the real one, and topological protection). A further note `notes/kitaev_error_taxonomy.tex` answers which physical Kitaev-chain errors destroy the topological phase — robust parity-even charge noise vs fatal parity-odd quasiparticle poisoning — and shows that the device's own NISQ errors are the *same* taxonomy under Jordan-Wigner ($T_1$ relaxation = poisoning, $T_2$ dephasing = charge noise), while the topological protection does not transfer to the qubit emulation.
